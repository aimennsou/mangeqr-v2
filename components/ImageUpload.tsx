...
type FileType = 'image/jpeg' | 'image/png' | 'default'
...
export default function FileUpload({
  ...
  onUploadComplete = () => { toast.success('Image uploaded successfully!') },
  onUploadError = (text: string) => { toast.error(text) },
  onDelete = () => { toast.success('Image deleted successfully!') },
  multiple = false,
}: FileUploadProps) {
  ...
  const renderUploadArea = () => (
    <div className="text-center">
      ...
      <div className="mt-4">
        ...
        ou glissez-déposez une image{multiple ? '(s)' : ''} ici
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
         {acceptedFileTypes.join(', ')} jusqu'à {maxSizeInMB}MB
        </p>
      </div>
    </div>
  )
  ...
  const renderUploadProgress = () => (
    <motion.div
      className="w-full pt-4 rounded-lg"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
    >
      <div className="flex items-center justify-between p-2 sm:p-3 bg-muted rounded-lg mt-4  overflow-hidden">
        ...
        >
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-base sm:text-lg font-semibold">Uploaded Images</CardTitle>
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 px-4 sm:px-0">
      <Card>
        ...
        <CardContent>
          {currentUpload ? renderUploadProgress() : renderUploadArea()}
          {renderUploadedFiles()}
        </CardContent>
      </Card>
    </div>
